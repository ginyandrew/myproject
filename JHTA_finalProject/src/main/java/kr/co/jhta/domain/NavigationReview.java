...
public class NavigationReview {
	...
	private CriteriaReview cReview;	//Criteria를 여기에 불러와서 currentBlock, pagesPerBlock값 등을 끌어와 쓴다.
	
	public NavigationReview(int totalRows, CriteriaReview cReview){
		...
		totalPages = (int)(Math.ceil((double)totalRows/cReview.getRowsPerPage()));
		
		//한 화면에 페이지 3개씩 보여주려면>> 전체 페이지15 나누기 3개 = 5개 블록 을 계산한 것. (totalBlock =전체 블록) 어떤 페이지가 몇 블락에 속한건지도 계산한다.
		totalBlock = (int)(Math.ceil((double)totalPages/cReview.getPagesPerBlock()));
		
		//한 블록에 첫번째가 몇번째 페이지인지 나온다. 한 블록에 3페이지씩 나온다 하면, 2번째 블록은 (2-1)*3+1 하면 4페이지부터 시작. 
		startPage = (int) ((cReview.getCurrentBlockNo()-1)* cReview.getPagesPerBlock() +1);
		
		// totalblock *3 하면 한 블록에 마지막이 몇번째페이지인지 나온다. 2번째 블록의 마지막페이지는 2*3 하면 6페이지임.
		endPage = (int) (cReview.getCurrentBlockNo()* cReview.getPagesPerBlock());
		
		
		if(cReview.getCurrentBlockNo() == totalBlock){
			endPage = totalPages;
		}
		
		//이전 페이지로 이동하는 if문. :현재 페이지가 1보다는 커야 이전페이지로 이동할 수 있다.
		if(cReview.getPageNo() >1 ){		
			prev = cReview.getPageNo() -1;
		}
		
		//다음 페이지로 이동하는 if문. 전체 페이지수보다 작은 페이지로만 이동가능함. 전체가 15페이지인데 16으론 이동할 수는 없다. 
		if(cReview.getPageNo() <totalPages){
			next = cReview.getPageNo() +1;
		}
	}
	...
	public CriteriaReview getCriteria() {
		return cReview;
	}
