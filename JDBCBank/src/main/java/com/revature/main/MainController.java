...
public class MainController {
	...
	public static void superUserMainMenu() {
		...
		case 2: // 2: delete user
			...
			ifRightOption = checkIfRightNumberFormat(sc.nextLine().trim());		// return false if it's not number.
			
			int userId = parsedNumber;
			if (ifRightOption && userDao.ifUserExist(userId) > 0) {
				accountDao.deleteBankAccountByUser(userId);
				userDao.deleteUser(userId);
				lc.logInfo("Deleting a user was successfullly completed!! \r\n");
			}
